...
@Repository
public interface RecetaRepositoryTest extends JpaRepository<Receta, Long> {
    ...
    // --- NOMBRE (versión segura para H2 — sin ESCAPE) ---
    @Query("""
        SELECT r FROM Receta r
        WHERE LOWER(r.nombre) LIKE LOWER(CONCAT('%', :nombre, '%'))
        """)
    List<Receta> buscarPorNombre(@Param("nombre") String nombre);
